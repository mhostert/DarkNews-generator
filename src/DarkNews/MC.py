...
class MC_events:
    ...
    def __init__(self, experiment, bsm_model, **kwargs):
        ...
        if "nuclear_target" in scope:
            ...
            logger.warning(
                "No target passed to MC_events, using first entry in experiment class instead."
            )
        ...
        if scope["decay_product"] == "e+e-":
            ...
            DECAY_PRODUCTS = (
                f"{self.decay_product.invert().name} {self.decay_product.name}"
            )
            ...
            logger.error(
                f"Error! Could not find decay product: {scope['decay_product']}"
            )
            raise ValueError
        ...
        if self.ups_case.Ethreshold > self.experiment.ERANGE[-1]:
            logger.error(
                f"Particle {self.nu_upscattered.name} is too heavy to be produced in the energy range of experiment {self.experiment.NAME}"
            )
            raise ValueError

        if self.ups_case.m_ups != self.decay_case.m_parent:
            logger.error(
                f"Error! Mass of HNL produced in neutrino scattering, m_ups = {self.ups_case.m_upscattered} GeV, different from that of parent HNL, m_parent = { self.decay_case.m_parent} GeV."
            )
            raise ValueError

        # process being considered
        self.underl_process_name = f"{self.nu_projectile.name} {self.ups_case.target.name} --> {self.nu_upscattered.name}  {self.ups_case.target.name} --> {self.nu_outgoing.name} {DECAY_PRODUCTS} {self.ups_case.target.name}"
    ...
    def get_total_xsec(self, Enu):
        ...
        tot_xsec = (
            integrals["diff_xsec"].mean
            * const.attobarn_to_cm2
            * batch_f.norm["diff_xsec"]
        )
        ...
        if self.scope["scattering_regime"] == "coherent":
            ...
            logger.error(
                f"Scattering regime {self.scope['scattering_regime']} not supported."
            )
    ...
    def get_MC_events(self):
        ...
        logger.debug(
            f"Vegas results for diff_event_rate: {weights['diff_event_rate'].sum()}"
        )
        logger.debug(
            f"Vegas results for diff_flux_avg_xsec: {weights['diff_flux_avg_xsec'].sum()}"
        )
        ...
        for decay_step in (k for k in batch_f.int_dic.keys() if "decay_rate" in k):
            ...
            df_gen[f"w_{decay_step}".replace("diff_", "")] = (
                weights[decay_step] * batch_f.norm[decay_step]
            )
        ...
        exposure = (
            self.experiment.NUMBER_OF_TARGETS[self.nuclear_target.name]
            * self.experiment.POTS
        )

        # differential rate weights
        df_gen["w_event_rate"] = (
            weights["diff_event_rate"]
            * const.attobarn_to_cm2
            / decay_rates
            * target_multiplicity
            * exposure
            * batch_f.norm["diff_event_rate"]
        )

        # flux averaged xsec weights (neglecting kinematics of decay)
        df_gen["w_flux_avg_xsec"] = (
            weights["diff_flux_avg_xsec"]
            * const.attobarn_to_cm2
            * target_multiplicity
            * exposure
            * batch_f.norm["diff_flux_avg_xsec"]
        )
        ...
        df_gen.attrs[f"{self.nu_upscattered.name}_ctau0"] = const.get_decay_rate_in_cm(
            df_gen[f"w_decay_rate_0"].sum()
        )
        ...
        if self.scope["enforce_prompt"]:
            geom.place_decay(
                df_gen, "P_decay_N_parent", l_decay_proper_cm=0.0, label="pos_decay"
            )
        else:
            # decay only the first mother (typically the HNL produced)
            logger.info(
                f"Parent {self.ups_case.nu_upscattered.name} proper decay length: {df_gen.attrs[f'{self.nu_upscattered.name}_ctau0']:.3E} cm.\n"
            )
            geom.place_decay(
                df_gen,
                "P_decay_N_parent",
        ...
        # print final result
        logger.info(
            f"Predicted ({df_gen['w_event_rate'].sum():.3g} +/- {np.sqrt((df_gen['w_event_rate']**2).sum()):.3g}) events.\n"
        )
...
def get_merged_MC_output(df1, df2):
    ...
    if not df.attrs:
        logger.debug(
            f"DEBUG: Forcing the storage of the df.attrs using the first dataframe. This is done automatically for newer versions of pandas."
        )
        df.attrs = df1.attrs
...
def run_vegas(
    batch_f, integ, NINT=10, NEVAL=1000, NINT_warmup=10, NEVAL_warmup=1000, **kwargs
):
    """
    ...
    return integ(
        batch_f, nitn=NINT, neval=NEVAL, uses_jac=True, **kwargs
    )  # , adapt=False)
