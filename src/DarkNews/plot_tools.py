...
def batch_plot(df, PATH, title="Dark News"):
    ...
    if "P_decay_ell_minus" in df.columns.levels[0]:
        ...
        # HISTOGRAMS 1D ####################################################
        # momentum exchange
        histogram1D(PATH / "1D_E_lp.pdf", df["E_lp"], w, r"$E_{\ell^+}$/GeV", title, 100, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_E_lm.pdf", df["E_lm"], w, r"$E_{\ell^-}$/GeV", title, 100, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_E_tot.pdf", df["E_tot"], w, r"$(E_{\ell^-}+E_{\ell^+})$/GeV", title, 100, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_E_nu_QEreco.pdf", df["E_nu_reco"], w, r"$E_\nu^{\rm QE-reco}/$GeV", title, 20, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_E_leading.pdf", df["E_leading"], w, r"$E_{\rm leading}$/GeV", title, 100, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_E_subleading.pdf", df["E_subleading"], w, r"$E_{\rm subleading}$/GeV", title, 100, TMIN=0, TMAX=1.5, **args)
        histogram1D(PATH / "1D_cost_sum.pdf", df["costheta_sum"], w, r"$\cos(\theta_{(\ell \ell)\nu_{\rm proj}})$", title, 20, **args)
        histogram1D(PATH / "1D_theta_sum.pdf", df["theta_sum"], w, r"$\theta_{(\ell \ell) \nu_{\rm proj}}$", title, 20, TMIN=0, TMAX=60, **args)
        ...
        histogram1D(PATH / "1D_deltatheta.pdf", df["Delta_theta"], w, r"$\Delta \theta_{\ell^+ \ell^-}$ degrees", title, 40, TMIN=0, TMAX=60, **args)
