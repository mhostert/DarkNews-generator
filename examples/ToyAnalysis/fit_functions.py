...
#from ToyAnalysis import logger as toy_logger
import logger as toy_logger
...
data_MB_source = {'Enu' : hist_plot.get_data_MB(varplot='reco_Enu',loc=loc), 'angle' : hist_plot.get_data_MB(varplot='reco_angle',loc=loc)}
...
def chi2_binned_rate_3p1(df, couplings, coupling_factor, back_MC,D, sys=[0.1,0.1],type_fit='angle',decay_limit = 10000):
    ...
    if type_fit=='angle':
    	histograms = hist_plot.get_histogram1D(df_decay, NEVENTS=total_Nevent_MB, varplot='reco_angle',loc='../')
    else:
    	histograms = hist_plot.get_histogram1D(df_decay, NEVENTS=total_Nevent_MB, varplot='reco_Enu',loc='../')
    NP_MC = histograms[0]
...
class exp_plotter:
	...
	def compute_chi2_grid(self, dataset, type_fit, sys, back_MC,D,num=500):
		...
		if type_fit=='Enu':
			histograms = hist_plot.get_histogram1D(bag_reco_MB, NEVENTS=total_Nevent_MB, varplot='reco_Enu',loc=loc)
			if self.mode_chi2:
				self.dof_enu = 6.7
			dof = self.dof_enu
		elif type_fit=='angle':
			histograms = hist_plot.get_histogram1D(bag_reco_MB, NEVENTS=total_Nevent_MB, varplot='reco_angle',loc=loc)
			dof = self.dof_angle
	...
	def plot_bf_distribution(self, fit_source='Enu', kde=False, rounding=2):
		...
		hist_plot.plot_all_rates(df, fit_source, Nevents=nevents, title=title, plot_muB = False, path=PATH,loc='../')
	
	
	def compute_xsecs(self,m4, mzprime, gD=1.0, epsilon=1e-2, Umu4=1e-6, UD4=1.0):
		#########################
		# Set BSM parameters
		BSMparams = model.Model()
		BSMparams.gD = gD
		...
		BSMparams.HNLtype = self.neutrino_type
		...
		for target in TARGETS:
			for regime in REGIMES:
				for hel in HEL_CASES:
					...
					myMCs[key] = MC.MC_events(  BSMparams, 
...
# FUNCTIONS TO PLOT CROSS SECTIONS IN A MORE FREE WAY


def compute_xsecs(m4, mzprime, gD=1.0, epsilon=1e-2, Umu4=1e-6, UD4=1.0, neutrino_type='dirac'):
	#########################
	# Set BSM parameters
	BSMparams = model.Model()
	BSMparams.gD = gD
	...
	BSMparams.m4 = m4
	BSMparams.mzprime = mzprime
	BSMparams.HNLtype = neutrino_type
	BSMparams.set_high_level_variables()
...
def compare_xsection(m4=0.3 ,mzprime=1.25 ,definition_number=500, regime = 'coherent', gD=1.0, epsilon=1e-2, Umu4=1e-6, UD4=1.0, title_plot=""):
	
	xsecs_d = compute_xsecs(m4, mzprime, gD=gD, epsilon=epsilon, Umu4=Umu4, UD4=UD4, neutrino_type='dirac')
	xsecs_m = compute_xsecs(m4, mzprime, gD=gD, epsilon=epsilon, Umu4=Umu4, UD4=UD4, neutrino_type='majorana')
	
	theseMCs_d = xsecs_d[0]
	mc_lowT_d = xsecs_d[1]
	
	theseMCs_m = xsecs_m[0]
	mc_lowT_m = xsecs_m[1]
	
	ts_d = mc_lowT_d.Ethreshold
	ts_m = mc_lowT_m.Ethreshold
	...
	fig, ax = hist_plot.std_fig()
	enu_axis = np.geomspace(threshold*0.99, 5, 100)
	...
	for key, mc in theseMCs_d.items():
		if regime in key:
			if regime == 'coherent' and "H1" in key:
				continue
			else:
	...
	for key, mc in theseMCs_m.items():
		if regime in key:
			if regime == 'coherent' and "H1" in key:
				continue
			else:
	...
	ax.set_title(fr'$m_{{Z^\prime}}= {mzprime:.2f}$ GeV,  $m_4 = {m4*1e3:.0f}$ MeV' + ', Dirac vs Majorana', fontsize=12)
	ax.set_yscale("log")
	ax.set_xscale("log")
	...
	fig.savefig(f"plots_xsec/xsec_3+1_{mzprime:.2f}_{m4*1e3:.0f}_{regime}_comparing" + title_plot + ".pdf",dpi=400)	
